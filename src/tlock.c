...
#define CLEAN_TAB_ENTRY(s) for (i = 0; s >= 0 && s < 2 && i < STRING_LIMIT; i++) \
      ...
      const char* passwd)
{
	...
	syslog(
	LOG_NOTICE, "tlock: authentication : returned %d\n", ret);
	...
	} else
	{
		...
		if (--attempt < 1)
		{
			syslog(
					LOG_ALERT,
					"Ended tlock with incorrect match!");
			closelog();
			exit(0);
		}
#endif
...
static pid_t getPidAtom(struct aXInfo* xinfo)
{
	...
	if (XGetWindowProperty(xinfo->display, xinfo->root[0], xinfo->pid_atom, 0L, 1L,
	False,
	XA_CARDINAL, &ret_type, &ret_fmt, &nr_read, &nr_bytes_left, (unsigned char**) &ret_data) == Success && ret_type != None && ret_data)
	{
		pid_t pid = *ret_data;
		XFree(ret_data);
		return pid;
